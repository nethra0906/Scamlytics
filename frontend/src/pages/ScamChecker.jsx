...
export default function ScamChecker() {
  ...
  const analyzeAudio = async () => {
    ...
    try {
      ...
      setResult({
        ...
        recommended_action: res.data.is_spoofed ? "Flag as fraudulent interaction." : "Proceed with standard verification.",
        note: res.data.note
      });
    } catch (e) {
      alert("Error: " + e.message);
  ...
  const handleGenerateReport = async () => {
    if (!result || result.type !== 'text' || !result.id) {
      alert("Report generation is only available for text analysis currently.");
      return;
    }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-surface-card p-1 rounded-lg border border-surface-border w-max">
        ...
        <button 
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === "audio" ? "bg-surface-elevated text-text-primary" : "text-text-muted hover:text-text-secondary"}`}
        >
      ...
      <div className="grid md:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-surface-card border border-surface-border rounded-xl p-6 shadow-sm">
          {activeTab === "text" ? (
            <div className="space-y-4">
              ...
              >
                {loading ? <><div className="w-4 h-4 border-2 border-surface-base border-t-transparent rounded-full animate-spin"></div> ANALYZING...</> : "ANALYZE THREAT"}
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-accent text-surface-base hover:bg-accent-hover py-3 rounded-md font-mono font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
              >
                {loading ? <><div className="w-4 h-4 border-2 border-surface-base border-t-transparent rounded-full animate-spin"></div> PROCESSING AUDIO...</> : "ANALYZE DEEPFAKE SIGNATURE"}
              </button>
            </div>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          {result && (
            ...
            >
              {/* Decorative background pulse for high risk */}
              {result.risk_level === "HIGH" && (
                <div className="absolute inset-0 bg-alert-high/5 animate-pulse pointer-events-none"></div>
              )}
              ...
              <div className="flex flex-col items-center mb-8 relative">
                <svg className="w-48 h-24" viewBox="0 0 100 50">
                  ...
                  <motion.path 
                    ...
                    fill="none" 
                    stroke={result.risk_level === "HIGH" ? "var(--color-alert-high)" : result.risk_level === "MEDIUM" ? "var(--color-accent)" : "var(--color-alert-low)"}
                    strokeWidth="8" 
                    strokeLinecap="round" 
                  />
                </svg>
              ...
              {/* Generate NCRB Report (Only Police can generate reports, and only for text currently) */}
              {role === "Police" && result.type === 'text' && result.id && (
                <div className="mt-8 pt-6 border-t border-surface-border">
                  ...
                  <p className="text-center text-[10px] text-text-muted mt-2 font-mono uppercase">Simulates Cybercrime.gov.in integration</p>
                </div>
              )}
            </motion.div>
          )}
