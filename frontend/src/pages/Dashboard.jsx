...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        toast.error(
          err?.response?.data?.detail || err.message || "Failed to load dashboard statistics.",
          "Dashboard Error"
        );
      })
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Module Grid — skeleton while loading */}
      {loading ? (
          ...
          {visibleCards.map((c) => (
            <motion.div key={c.title} variants={itemVariants}>
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 h-0.5 bg-accent scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500" />
              </Link>
            </motion.div>
          ))}
        </motion.div>
              ...
              <div className="bg-surface-card rounded-xl border border-surface-border overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  {stats.recent_activity.scams.length === 0 && (
                    <p className="text-text-muted text-sm font-mono py-2">NO RECENT DATA</p>
                  )}
                </div>
              </div>
